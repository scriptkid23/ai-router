...
class PageWorker:
    ...
    async def _run(self) -> None:
        attach_listeners(self._page, self._channel)
    ...
    async def _dom_snapshot(self, page: Page) -> dict:
        ...
        try:
            ...
            pass
        err = None
        for marker in GEMINI_ERROR_MARKERS:
            if marker in body:
                err = body[:200]
                break
        return {
    ...
    async def _execute_job(self, job: AskJob) -> str:
        ...
        try:
            answer = await executor.run(planner.plan(job))
            trace(
                "job_done",
                page_id=self._page_id,
                job_id=job.job_id,
            ...
            if exc.code == "GEMINI_ERROR":
                ...
                answer = await executor.run(planner.plan(job, recovery=True))
                trace(
                    "job_done",
                    page_id=self._page_id,
                    job_id=job.job_id,
